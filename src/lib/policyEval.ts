...
function deriveFieldValue(obj: EvaluableObject, field: string): string | number | boolean | undefined {
  ...
  switch (field) {
    ...
    case "cert_expiry_days":
      return obj.daysToExpiry;
    case "is_self_signed":
      return !!obj.isSelfSigned;
    case "revocation_status":
      return obj.revocationStatus || "Valid";
    case "protocol_version":
      return obj.protocolVersion;
    case "cipher_suite":
      return obj.cipherSuite;
    case "protection_level":
      return obj.protectionLevel;
    case "has_expiry":
      return obj.hasExpiry !== undefined ? obj.hasExpiry : obj.daysToExpiry !== undefined && obj.daysToExpiry >= 0;
    case "rotation_enabled":
      return (obj.rotationFrequency || "").toLowerCase() !== "never";
...
function compare(actual: string | number | boolean | undefined, operator: string, raw: string): boolean {
  ...
  switch (operator) {
    case "eq":
      return aStr === raw.trim().toLowerCase();
    case "neq":
      return aStr !== raw.trim().toLowerCase();
    case "lt":
    ...
    case "gte":
      return !isNaN(aNum) && !isNaN(vNum) && aNum >= vNum;
    case "contains":
      return aStr.includes(raw.trim().toLowerCase());
    case "ncontains":
      return !aStr.includes(raw.trim().toLowerCase());
    case "in":
      return list.includes(aStr);
    case "nin":
      return !list.includes(aStr);
    default:
      return false;
...
function evalGroup(obj: EvaluableObject, group: EvalGroup): boolean {
  ...
  return group.innerLogic === "OR" ? rows.some((r) => evalRow(obj, r)) : rows.every((r) => evalRow(obj, r));
}

/** True if the object VIOLATES the policy (its conditions match). */
export function objectViolatesPolicy(obj: EvaluableObject, groups: EvalGroup[], groupLogic: "AND" | "OR"): boolean {
  const active = groups.filter((g) => g.rows.some((r) => r.field && r.operator));
  if (!active.length) return false;
  return groupLogic === "OR" ? active.some((g) => evalGroup(obj, g)) : active.every((g) => evalGroup(obj, g));
}
...
export function explainObject(
  obj: EvaluableObject,
  groups: EvalGroup[],
): Array<{ field: string; operator: string; value: string; pass: boolean; actual: string }> {
