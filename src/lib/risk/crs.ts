...
function complianceRaw(a: CryptoAsset): { v: number; why: string } {
  ...
  switch (a.pqcRisk) {
    case "Critical":
      return { v: 90, why: "PQC-vulnerable · NIST 2030 deadline" };
    case "High":
      return { v: 65, why: "PQC-at-risk · migration window open" };
    case "Medium":
      return { v: 40, why: "PQC-watch · monitor for downgrade" };
    case "Low":
      return { v: 20, why: "Compliant with current standards" };
    case "Safe":
      return { v: 5, why: "PQC-safe algorithm" };
  }
}
...
export function getRiskClass(a: CryptoAsset): RiskClass {
  ...
  const isQuantum = QUANTUM_VULNERABLE.some((alg) => a.algorithm.includes(alg.split("-")[0]) || a.algorithm === alg);
  const isOperational =
    a.status === "Expired" ||
    a.status === "Orphaned" ||
