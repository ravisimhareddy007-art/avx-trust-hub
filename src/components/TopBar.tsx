...
import { Bell, Infinity as InfinityIcon, User, HelpCircle, AlertTriangle, Shield, X, RefreshCw } from "lucide-react";
import { toast } from "sonner";
import HelpDrawer from "@/components/help/HelpDrawer";

const breadcrumbMap: Record<string, string> = {
  dashboards: "Trust Posture & Risk Intelligence",
...
export default function TopBar() {
  ...
  return (
    <div className="h-14 bg-card border-b border-border flex items-center px-4 gap-4 flex-shrink-0">
      {/* Breadcrumb */}
      <div className="text-sm font-medium text-foreground">{breadcrumbMap[currentPage] || currentPage}</div>
      ...
      <div className="flex items-center gap-3">
        ...
        >
          ...
          Infinity Intelligence
          {activeUndo > 0 && (
            <span className="absolute -top-1 -right-1 w-4 h-4 bg-amber rounded-full text-[9px] text-background flex items-center justify-center font-bold">
              {activeUndo}
            </span>
          )}
        </button>
        ...
        <div className="relative">
          <button onClick={handleBellClick} className="relative p-2 rounded-lg hover:bg-muted transition-colors">
            ...
            <div className="absolute right-0 top-full mt-1 w-96 bg-card border border-border rounded-lg shadow-xl z-50">
              <div className="flex items-center justify-between px-4 py-3 border-b border-border">
                ...
                <div className="flex items-center gap-2">
                  {myUnread > 0 && (
                    <button onClick={markAllRead} className="text-[10px] text-teal hover:underline">
                      Mark all read
                    </button>
                  )}
                    ...
                    {escalationsForMe.map((n) => (
                      ...
                      >
                        <div className="flex items-start gap-2.5">
                          <div
                            className={`w-7 h-7 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5 ${!n.read ? "bg-coral/15" : "bg-muted"}`}
                          >
                            <AlertTriangle
                              className={`w-3.5 h-3.5 ${!n.read ? "text-coral" : "text-muted-foreground"}`}
                            />
                          </div>
                          <div className="flex-1 min-w-0">
                            ...
                            <p
                              className={`text-[10px] font-medium ${sevColor[n.violationSeverity] || "text-muted-foreground"}`}
                            >
