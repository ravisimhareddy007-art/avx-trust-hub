        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-[80%] bg-card border-l border-border shadow-2xl h-full overflow-y-auto animate-slide-in-right">
            ...
            <div className="p-4 space-y-4">
              ...
              {(() => {
                ...
                return (
                  <div className="grid grid-cols-5 gap-3">
                    {[
                      { label: "Objects", value: ids.length, color: "text-foreground" },
                      { label: "IT Assets", value: assetCount, color: "text-foreground" },
                      { label: "Policies", value: selectedGroup.policyCount, color: "text-foreground" },
                      {
                        label: "With Violations",
                        value: withViolations,
                        color: withViolations > 0 ? "text-coral" : "text-teal",
                      },
                      { label: "PQC Critical", value: critical, color: critical > 0 ? "text-coral" : "text-teal" },
                    ].map((s) => (
              ...
              <div className="bg-card rounded-lg border border-border overflow-hidden">
                <div className="px-3 py-2 border-b border-border flex items-center justify-between">
                  <p className="text-xs font-semibold text-foreground">
                    Objects ({getIdentities(selectedGroup).length})
                  </p>
                  <span className="text-[9px] text-muted-foreground">
                    Click a row to view it in the asset inventory
                  </span>
                </div>
                <div className="overflow-x-auto scrollbar-thin">
                  <table className="w-full text-xs">
                    <thead className="bg-secondary/50">
                      <tr className="border-b border-border">
                        <th className="text-left py-1.5 px-2 font-medium text-muted-foreground">Object</th>
                        <th className="text-left py-1.5 px-2 font-medium text-muted-foreground">IT Asset</th>
                    ...
                    <tbody>
                      {getIdentities(selectedGroup).map((co) => (
                        <tr
                          ...
                          onClick={() => viewInInventory(co.id)}
                          title="View this object in the asset inventory"
                        >
                          <td className="py-1.5 px-2 font-medium text-foreground truncate max-w-[180px]">{co.name}</td>
                          <td className="py-1.5 px-2 text-muted-foreground text-[10px] truncate max-w-[140px]">
                            {co.application || "\u2014"}
                          </td>
              ...
              {/* Policy posture (read-only, monitoring) */}
              <div className="bg-card border border-border rounded-lg overflow-hidden">
                <div className="px-3 py-2 border-b border-border flex items-center justify-between">
                  <p className="text-xs font-semibold">Policy posture ({selectedGroup.policies.length})</p>
                  <span className="text-[9px] text-muted-foreground">
                    Read-only. Policies are attached from the policy builder.
                  </span>
                </div>
                {selectedGroup.policies.length > 0 ? (
                  ...
                  <div className="py-6 text-center text-[10px] text-muted-foreground">
                    No policies apply to this group yet.
                  </div>
                )}
              </div>
            </div>
