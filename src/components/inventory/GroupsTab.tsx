...
const NUMERIC_ATTRS = new Set(["Days to Expiry", "Validity Period", "Last Rotation", "Key Size"]);
const isNumericAttr = (attr: string) => NUMERIC_ATTRS.has(attr);

// Single source of truth for evaluating one condition against one asset.
// Returns null when the attribute is not matchable, so callers can skip it
// honestly rather than silently treat it as a pass.
function evalConditionOnAsset(a: CryptoAsset, c: GroupCondition): boolean | null {
  if (!c.value) return null;
  const eq = (actual: string | undefined) => (c.operator === "not_equals" ? actual !== c.value : actual === c.value);
  switch (c.attribute) {
    case "Algorithm":
      return eq(a.algorithm);
    case "Environment":
      return eq(a.environment);
    case "Object Type":
      return eq(a.type);
    case "Issuing CA":
      return eq(a.caIssuer);
    case "Key Size":
      return eq(a.keyLength);
    case "Owner Team":
      return eq(a.team);
    case "Has Owner": {
      const hasOwner = a.owner !== "Unassigned";
      return c.value === "Yes" ? hasOwner : !hasOwner;
    }
    case "Days to Expiry": {
      const d = a.daysToExpiry;
...
function ConditionBuilder({
  ...
  onChange: (c: GroupCondition[]) => void;
}) {
  const allAttrs = Object.values(conditionAttributes).flatMap((cat) => cat.map((a) => a.label));
  const getValues = (attr: string) =>
    Object.values(conditionAttributes)
      .flatMap((cat) => cat)
      .find((a) => a.label === attr)?.values || [];

  const addCondition = () => {
    onChange([
      ...conditions,
      { id: `c-${Date.now()}`, attribute: allAttrs[0], operator: "equals", value: "", logic: "AND" },
    ]);
  };
  ...
  const updateCondition = (id: string, field: keyof GroupCondition, value: string) => {
    onChange(
      conditions.map((c) => {
        ...
        // Switching to a non-numeric attribute clears any < / > operator and the
        // now-mismatched value so the condition can never hold nonsense.
        if (field === "attribute" && !isNumericAttr(value)) {
          if (next.operator === "less_than" || next.operator === "greater_than") next.operator = "equals";
        }
        if (field === "attribute") next.value = "";
        return next;
      }),
    );
  };

  // Honor per-condition AND/OR via the shared evaluator.
  const evaluable = conditions.filter((c) => c.value);
  const unmatchable = evaluable.some((c) => mockAssets.length > 0 && evalConditionOnAsset(mockAssets[0], c) === null);
  const usesOr = evaluable.some((c) => c.logic === "OR");
  ...
  // is represented faithfully instead of collapsed to a single operator.
  const opSym = (op: string) =>
    op === "equals"
      ? "="
      : op === "not_equals"
        ? "\u2260"
        : op === "less_than"
          ...
          : op === "greater_than"
            ? ">"
            : op;
  const summary = evaluable
    .map((c, i) => `${i > 0 ? (c.logic || "AND") + " " : ""}${c.attribute} ${opSym(c.operator)} "${c.value}"`)
    .join(" ");
    ...
    <div className="space-y-3">
      {conditions.map((c, i) => (
        <div key={c.id}>
          {i > 0 && (
            <button
              onClick={() => updateCondition(c.id, "logic", c.logic === "AND" ? "OR" : "AND")}
              className="text-[10px] font-semibold text-teal px-2 py-0.5 rounded bg-teal/10 mb-1"
            >
              {c.logic || "AND"}
            </button>
          )}
          <div className="flex items-center gap-1.5">
            ...
            >
              ...
              {getValues(c.attribute).map((v) => (
                <option key={v}>{v}</option>
              ))}
            </select>
            <button onClick={() => removeCondition(c.id)} className="p-1 text-muted-foreground hover:text-coral">
              <X className="w-3 h-3" />
            </button>
          </div>
        </div>
      ))}
      <button onClick={addCondition} className="text-[10px] text-teal hover:underline flex items-center gap-1">
      ...
      {conditions.length > 0 && conditions.some((c) => c.value) && (
        <div className="bg-secondary/50 rounded p-2">
          ...
          {unmatchable && (
            <p className="text-[9px] text-amber mt-0.5">Some conditions are not yet reflected in this live count.</p>
          )}
        </div>
      )}
    </div>
...
export default function GroupsTab({ onCreateTicket, onOpenPolicyDrawer }: Props) {
  ...
  const handleCreateGroup = () => {
    ...
    const newGroup: DynamicGroup = {
      ...
      ownerTeam: newGroupOwner,
      topAlgorithms: [],
      topEnvironments: [],
      topIssuers: [],
      typeBreakdown: [],
      posture: { ah: 50, ep: 50, pqr: 50, gc: 50, ait: 50 },
      trendData: Array(30).fill(45),
      aiNarrative: "",
      policies: [],
      remediationTasks: [],
    };
  ...
  const handleAcceptAISuggestion = (sg: Partial<DynamicGroup>) => {
    const newGroup: DynamicGroup = {
      ...
      typeBreakdown: [],
      posture: { ah: 50, ep: 50, pqr: 50, gc: 50, ait: 50 },
      trendData: Array(30).fill(sg.riskScore || 50),
  ...
  const handleNLParse = () => {
    ...
    if (lower.includes("production"))
      parsed.push({ id: "nl-1", attribute: "Environment", operator: "equals", value: "Production", logic: "AND" });
    if (lower.includes("tls") || lower.includes("certificate"))
      parsed.push({ id: "nl-2", attribute: "Object Type", operator: "equals", value: "TLS Certificate", logic: "AND" });
    if (lower.includes("weak") || lower.includes("rsa-2048"))
      parsed.push({ id: "nl-3", attribute: "Algorithm", operator: "equals", value: "RSA-2048", logic: "AND" });
    if (lower.includes("ssh"))
      parsed.push({ id: "nl-4", attribute: "Object Type", operator: "equals", value: "SSH Key", logic: "AND" });
    if (lower.includes("no owner") || lower.includes("unassigned"))
      parsed.push({ id: "nl-5", attribute: "Has Owner", operator: "equals", value: "No", logic: "AND" });
    if (parsed.length === 0)
      parsed.push({ id: "nl-0", attribute: "Environment", operator: "equals", value: "Production", logic: "AND" });
      ...
      <div className="flex items-center gap-2 p-3 border-b border-border flex-shrink-0">
        ...
        <div className="ml-auto flex items-center gap-2">
          ...
          <button
            ...
            className="flex items-center gap-1 px-2.5 py-1.5 rounded bg-teal text-primary-foreground text-[10px] font-medium hover:opacity-90"
          >
      ...
      <div className="flex-1 min-h-0 overflow-y-auto p-3 space-y-3">
          ...
          <div className="bg-teal/5 border border-teal/20 rounded-lg p-3 space-y-2">
            <div className="flex items-center justify-between">
              <p className="text-xs font-semibold text-teal flex items-center gap-1.5">
                <Sparkles className="w-3.5 h-3.5" /> AI-Suggested Groups
              </p>
              <button
                    ...
                    <tr key={sg.id} className="border-b border-teal/10">
                      ...
                      <td className="py-2 px-2">
                        <div className="flex gap-1">
                          <button
                            onClick={() => handleAcceptAISuggestion(sg)}
                            className="px-2 py-1 rounded text-[9px] font-medium bg-teal/10 text-teal hover:bg-teal/20"
                          >
                            Create
                          </button>
                          <button className="px-2 py-1 rounded text-[9px] text-muted-foreground hover:text-foreground hover:bg-secondary">
                            Dismiss
                          </button>
                        </div>
                      </td>
        ...
        <div className="bg-card rounded-lg border border-border overflow-hidden">
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="py-2.5 px-2 text-center">
                        <span
                          className={`inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold ${riskColor}`}
                        >
                          {g.riskScore}
                        </span>
                      </td>
                      ...
                      <td className="py-2.5 px-2">
                        <div className="flex items-center gap-1.5">
                          <div className="w-14 h-1.5 bg-secondary rounded-full overflow-hidden">
                            <div className="h-full bg-teal rounded-full" style={{ width: `${g.policyCoverage}%` }} />
                          </div>
          ...
          {filteredGroups.length === 0 && (
            <div className="py-12 text-center text-sm text-muted-foreground">No groups match your search.</div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex">
          ...
          <div className="w-[80%] bg-card border-l border-border shadow-2xl h-full overflow-y-auto animate-slide-in-right">
            ...
            <div className="sticky top-0 bg-card border-b border-border px-4 py-3 flex items-center gap-3 z-10">
              ...
              <span
                className={`px-2 py-0.5 rounded text-[10px] font-semibold ${selectedGroup.type === "Dynamic" ? "bg-purple/10 text-purple" : "bg-secondary text-muted-foreground"}`}
              >
                {selectedGroup.type}
              </span>
              ...
              <div className="ml-auto flex items-center gap-2">
                <button
                  onClick={() => toast.success("Re-evaluating...")}
                ...
                <button
                  onClick={() => onOpenPolicyDrawer(selectedGroup.id, selectedGroup.name)}
                  className="px-2 py-1 rounded text-[10px] font-medium bg-teal text-primary-foreground hover:opacity-90"
                >
                  Attach Policy
                </button>
            ...
            <div className="p-4 space-y-4">
              ...
              <div className="grid grid-cols-5 gap-3">
                {[
                  {
                    label: "Risk Score",
                    value: selectedGroup.riskScore,
                    color:
                      selectedGroup.riskScore > 70
                        ? "text-coral"
                        : selectedGroup.riskScore > 40
                          ? "text-amber"
                          : "text-teal",
                  },
                  ...
                  {
                    label: "Coverage",
                    value: `${selectedGroup.policyCoverage}%`,
                    color: selectedGroup.policyCoverage > 80 ? "text-teal" : "text-amber",
                  },
                  {
                    label: "Open Tasks",
                    value: selectedGroup.remediationTasks.length,
                    color: selectedGroup.remediationTasks.length > 0 ? "text-coral" : "text-teal",
                  },
                ].map((s) => (
              ...
              <div className="bg-card rounded-lg border border-border overflow-hidden">
                <div className="px-3 py-2 border-b border-border flex items-center justify-between">
                  <p className="text-xs font-semibold text-foreground">
                    Identities ({getIdentities(selectedGroup).length})
                  </p>
                  <div className="flex gap-1">
                    {["Export", "Assign Owner"].map((a) => (
                      <button
                        key={a}
                        onClick={() => toast.success(`${a} initiated`)}
                        className="px-2 py-1 rounded text-[9px] border border-border text-muted-foreground hover:text-foreground hover:bg-secondary"
                      >
                        {a}
                      </button>
                    ))}
                      ...
                      {getIdentities(selectedGroup).map((co) => (
                        <React.Fragment key={co.id}>
                          ...
                          >
                            ...
                            <td className="py-1.5 px-2 font-medium text-foreground truncate max-w-[200px]">
                              {co.name}
                            </td>
                            ...
                            <td className="py-1.5 px-2 text-center">
                              {co.policyViolations === 0 ? (
                                <Shield className="w-3.5 h-3.5 text-teal inline" />
                              ) : (
                                <ShieldOff className="w-3.5 h-3.5 text-coral inline" />
                              )}
                            </td>
                            <td className="py-1.5 px-2 text-center">
                              {co.policyViolations > 0 && (
                                <span className="inline-flex items-center justify-center w-4 h-4 rounded-full bg-coral/10 text-coral text-[9px] font-bold">
                                  {co.policyViolations}
                                </span>
                              )}
                            </td>
                              ...
                              <td colSpan={9} className="p-0">
                                <div className="bg-secondary/30 px-4 py-2 grid grid-cols-3 gap-3">
                                  <div>
                                    ...
                                    {co.policyViolations > 0 ? (
                                      <div className="flex items-center gap-1 text-[10px]">
                                        <span className="px-1 py-0.5 rounded bg-amber/10 text-amber text-[9px]">
                                          Warn
                                        </span>
                                  ...
                                  <div>
                                    ...
                                    {co.policyViolations > 0 ? (
                                      <p className="text-[10px] text-coral">{co.policyViolations} active violations</p>
                                    ) : (
                                    ...
                                    <div className="flex flex-wrap gap-1">
                                      {["Renew", "Create Ticket"].map((a) => (
                                        <button
                                          ...
                                          className="px-1.5 py-0.5 rounded text-[9px] border border-border text-muted-foreground hover:text-foreground"
                                        >
                                          {a}
                                        </button>
                                      ))}
                ...
                <div className="bg-card border border-border rounded-lg overflow-hidden">
                  ...
                  {selectedGroup.policies.length > 0 ? (
                      ...
                      <tbody>
                        {selectedGroup.policies.map((p) => (
                          <tr key={p.id} className="border-b border-border">
                            ...
                            <td className="py-2 px-2">
                              <span
                                className={`px-1.5 py-0.5 rounded text-[9px] font-medium ${p.enforcementMode === "Enforce" ? "bg-coral/10 text-coral" : "bg-amber/10 text-amber"}`}
                              >
                                {p.enforcementMode}
                              </span>
                            </td>
                            ...
                            <td className="py-2 px-2 text-center">
                              {p.violations > 0 ? (
                                <span className="text-coral font-medium">{p.violations}</span>
                              ) : (
                    ...
                    <div className="py-6 text-center">
                      ...
                      <button
                        onClick={() => onOpenPolicyDrawer(selectedGroup.id, selectedGroup.name)}
                        className="px-3 py-1.5 rounded text-[10px] font-medium bg-teal text-primary-foreground hover:opacity-90"
                      >
                        Attach Policy
                      </button>
                    </div>
                ...
                <div className="bg-card border border-border rounded-lg overflow-hidden">
                  <div className="px-3 py-2 border-b border-border flex items-center justify-between">
                    ...
                    {selectedGroup.remediationTasks.some((t) => t.severity === "Critical") && (
                      <button
                        onClick={() => toast.success("Auto-remediable tasks queued")}
                        className="text-[10px] text-coral hover:underline"
                      >
                        Execute Auto-Remediable
                      </button>
                    )}
                  </div>
                      ...
                      <tbody>
                        {selectedGroup.remediationTasks.map((t) => (
                          <tr key={t.id} className="border-b border-border">
                            <td className="py-2 px-3 font-medium text-foreground truncate max-w-[150px]">
                              {t.objectName}
                            </td>
                            <td className="py-2 px-2 text-muted-foreground">{t.taskType}</td>
                            <td className="py-2 px-2">
                              <span
                                className={`w-1.5 h-1.5 rounded-full inline-block mr-1 ${t.severity === "Critical" ? "bg-coral" : t.severity === "High" ? "bg-amber" : "bg-purple"}`}
                              />
                              <span className="text-[10px]">{t.severity}</span>
                            </td>
                            <td className="py-2 px-2">
        ...
        <div className="space-y-4 max-h-[70vh] overflow-y-auto pr-1">
              ...
              {(["Dynamic", "Manual"] as const).map((t) => (
                ...
                >
                  ...
                  <p className="text-[9px] text-muted-foreground">
                    {t === "Dynamic" ? "Auto-maintained by conditions" : "Hand-curated selection"}
                  </p>
                </button>
              ))}
            </div>
          ...
          <div>
            ...
            <input
              ...
              placeholder="e.g. RSA-2048 Production Certs"
              className={`w-full px-3 py-2 bg-muted border rounded text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-teal ${duplicateName ? "border-coral" : "border-border"}`}
            />
            {duplicateName && <p className="text-[10px] text-coral mt-1">A group with this name already exists.</p>}
          </div>
          <div>
            ...
            <div>
              <div className="flex items-center gap-2 mb-2">
                ...
                <div className="flex gap-1 ml-auto">
                  <button
                    onClick={() => setNlTab("builder")}
                    className={`px-2 py-0.5 text-[9px] rounded ${nlTab === "builder" ? "bg-teal/10 text-teal font-medium" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    Builder
                  </button>
                  <button
                    onClick={() => setNlTab("describe")}
                    className={`px-2 py-0.5 text-[9px] rounded ${nlTab === "describe" ? "bg-teal/10 text-teal font-medium" : "text-muted-foreground hover:text-foreground"}`}
                  >
                    ✦ Describe It
                  </button>
                </div>
              ...
              {nlTab === "builder" ? (
                ...
                <div className="space-y-2">
                  <textarea
                    ...
                    rows={2}
                    placeholder="e.g. All production TLS certs using weak algorithms"
                    className="w-full px-3 py-2 bg-muted border border-border rounded text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-teal resize-none"
                  />
                  <button
                    onClick={handleNLParse}
                    disabled={!nlInput}
                    className="px-3 py-1.5 text-[10px] rounded bg-teal text-primary-foreground hover:opacity-90 disabled:opacity-50"
                  >
                    Parse with AI
                  </button>
                </div>
          ...
          <div className="flex gap-2 justify-end pt-2 border-t border-border">
            <button
              ...
              className="px-4 py-2 text-xs border border-border rounded hover:bg-secondary"
            >
              Cancel
            </button>
            <button
              ...
              disabled={!canCreate}
              title={
                newGroupType === "Dynamic" && completeConditions.length === 0
                  ? "Add at least one condition"
                  : duplicateName
                    ? "Name already in use"
                    : ""
              }
              className="px-4 py-2 text-xs rounded bg-teal text-primary-foreground hover:opacity-90 disabled:opacity-50"
            >
              Create Group
            </button>
          </div>
