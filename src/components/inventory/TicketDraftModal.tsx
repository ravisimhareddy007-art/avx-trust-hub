...
export function generateTicketDraft(asset: CryptoAsset, action: string): TicketDraft {
    ...
    action === "pqc" && isProd
      ...
      : critical7 && isProd
        ...
        : expiring && isProd
          ...
          : unowned && isProd
            ? "High"
            : asset.policyViolations > 0 && isProd
              ? "High"
              : "Medium";
  ...
  if (action === "renew" || action === "expiry") {
    return {
      ...
      affectedSystems: `${asset.dependencyCount} dependent system${asset.dependencyCount !== 1 ? "s" : ""} · Application: ${asset.application} · Infrastructure: ${asset.infrastructure}`,
  ...
  if (action === "rotate") {
    return {
      ...
      affectedSystems: `${asset.dependencyCount} dependent system${asset.dependencyCount !== 1 ? "s" : ""} · ${asset.application} · ${asset.infrastructure}`,
  ...
  if (action === "pqc") {
    ...
    return {
      ...
      affectedSystems: `${asset.dependencyCount} downstream system${asset.dependencyCount !== 1 ? "s" : ""} · ${asset.application} · ${asset.infrastructure}`,
...
interface Props {
  ...
  /** Pre-built draft for object classes that are not CryptoAssets (protocols, libraries). */
...
export default function TicketDraftModal({
  asset,
  action,
  onClose,
  onConfirm,
        ...
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border flex-shrink-0">
          ...
          <div>
            ...
            <p className="text-[10px] text-muted-foreground">
              {asset?.name ?? "Infrastructure asset"} · Review and confirm before creating
            </p>
          </div>
            ...
            <div className="p-5 space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  <span
                    className={`text-[10px] font-bold px-2 py-0.5 rounded border ${PRIORITY_STYLE[draft.priority]}`}
                  >
                    {draft.priority}
                  </span>
                ...
                <div className="space-y-1">
                  {draft.remediationSteps.map((step, i) => (
                    <div key={i} className="flex items-start gap-2">
                      <span className="text-[10px] font-bold text-muted-foreground w-5 flex-shrink-0 mt-1.5">
                        {i + 1}.
                      </span>
                      {editingStep === i ? (
                        <input
                  ...
                  {createdIncident && (
                    <div className="mt-2 p-2 rounded bg-teal/10 border border-teal/30 flex items-center gap-2">
                      ...
                      <p className="text-[10.5px] text-foreground">
                        Created in ServiceNow as{" "}
                        <span className="font-mono font-semibold text-teal">{createdIncident}</span>
        ...
        {!thinking && draft && (
          <div className="flex items-center justify-between px-5 py-4 border-t border-border flex-shrink-0">
            ...
            <div className="flex gap-2">
              <button
                onClick={onClose}
                className="px-4 py-2 text-xs rounded-lg border border-border hover:bg-secondary transition-colors"
              >
