...
export default function PQCMigrationPanel() {
  ...
  const { stageCounts, totalCritical, withPlan, topHndl } = useMemo(() => {
    ...
    critical.forEach((a) => {
      statusMap[qthStatusFor(a.id, idx++)];
      statusMap[qthStatusFor(a.id, idx - 1)]++;
    });
        ...
        <div className="flex items-center justify-between mb-1">
          {stageCounts.map((s, i) => (
            <div key={s.label} className="flex flex-col items-center flex-1 relative z-10">
              ...
              <span className="text-[10px] text-muted-foreground mt-1.5 text-center leading-tight w-20">{s.label}</span>
            </div>
          ))}
        </div>
        {/* Progress line */}
        <div
          className="relative h-1 mx-auto mt-[-38px] mb-8"
          style={{ width: "calc(100% - 80px)", marginLeft: 40, marginRight: 40 }}
        >
          <div className="absolute inset-0 bg-border rounded-full" />
          <div
            className="absolute left-0 top-0 h-full bg-teal rounded-full transition-all"
            style={{ width: `${(furthestActive / 4) * 100}%` }}
          />
        </div>
        <p className="text-xs text-muted-foreground">
          <span className="text-teal font-semibold">{withPlan}</span> of{" "}
          <span className="font-semibold text-foreground">{totalCritical}</span> quantum-vulnerable objects have a
          migration plan
        </p>
      </div>
      ...
      <div>
        ...
        <div className="relative h-16">
          ...
          {TIMELINE.map((m) => (
            <div
              key={m.year}
              className="absolute"
              style={{ left: `${pct(m.year)}%`, transform: "translateX(-50%)", top: m.year === CURRENT_YEAR ? 0 : 12 }}
            >
              <div className="flex flex-col items-center">
        ...
        <div className="flex justify-between text-[9px] text-muted-foreground mt-1 px-1">
          {TIMELINE.map((m) => (
            <span
              key={m.year}
              className={`text-center max-w-[90px] leading-tight ${m.year === 2030 ? "text-coral font-semibold" : ""}`}
            >
              {m.label}
            </span>
          ))}
