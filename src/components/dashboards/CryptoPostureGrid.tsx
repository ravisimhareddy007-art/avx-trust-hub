...
const CORAL = 'hsl(var(--coral))', AMBER = 'hsl(var(--amber))', TEAL = 'hsl(var(--teal))', MUTED = 'hsl(var(--muted-foreground))';

export default function CryptoPostureGrid() {
  const { setCurrentPage, setFilters } = useNav();
  const go = (f: Record<string, string>) => { setFilters(f); setCurrentPage('inventory'); };
  ...
  const cExpired = certs.filter(a => a.status === 'Expired' || (typeof a.daysToExpiry === 'number' && a.daysToExpiry < 0)).length;
  ...
  const rotOverdue = (a: any) => { const f = String(a.rotationFrequency || ''); const y = f.match(/(\d+)\s*year/); if (y) return +y[1] >= 2; const d = f.match(/(\d+)\s*day/); if (d) return +d[1] > 365; return !a.rotationFrequency; };
  const quantum = (a: any) => a.pqcRisk === 'High' || a.pqcRisk === 'Critical' || /^(RSA|EC|ECDSA|ECDH|DSA|DH)\b/i.test(a.algorithm || '');
  ...
  const secUnrot = secrets.filter(a => { const d = ageDays(a.lastRotated); return d !== null && d > 90; }).length;
  const secOrphan = secrets.filter(unowned).length;
  const secStale = secrets.filter(a => !a.rotationFrequency).length;
  const secAct = secrets.filter(a => { const d = ageDays(a.lastRotated); return (d !== null && d > 90) || !a.owner || a.owner === 'Unassigned'; }).length;

  // Protocols donut by version
  const byVer = (fam: string, ver?: string) => mockProtocols.filter(p => p.family === fam && (ver ? p.version.startsWith(ver) : true)).length;
  const ssl = mockProtocols.filter(p => p.family === 'SSL').length;
  const t10 = byVer('TLS', '1.0'), t11 = byVer('TLS', '1.1'), t12 = byVer('TLS', '1.2'), t13 = byVer('TLS', '1.3');
  ...
  const certRows: PostureRow[] = [
    { label: 'Expiring in 7 days', count: c7 + cExpired, role: 'critical', onClick: () => go({ tab: 'identities', type: 'TLS Certificate', filterId: 'cert_expiring_7d' }) },
    { label: 'Expiring in 30 days', count: c30, role: 'high', onClick: () => go({ tab: 'identities', type: 'TLS Certificate', filterId: 'cert_expiring_30d' }) },
    { label: 'Expiring in 90 days', count: c90, role: 'medium', onClick: () => go({ tab: 'identities', type: 'TLS Certificate' }) },
  ];
  const sshRows: PostureRow[] = [
    { label: 'Under 1 year', count: sUnder1, role: 'neutral', onClick: () => go({ tab: 'identities', type: 'SSH Key' }) },
  ...
  const encRows: PostureRow[] = [
    { label: 'Rotation disabled / overdue', count: eRot, role: 'critical', onClick: () => go({ tab: 'identities', type: 'Encryption Key' }) },
    { label: 'Quantum-vulnerable (RSA / ECC)', count: eQ, role: 'high', onClick: () => go({ tab: 'identities', type: 'Encryption Key' }) },
    { label: 'Software-protected (not HSM)', count: eSw, role: 'medium', onClick: () => go({ tab: 'identities', type: 'Encryption Key' }) },
    { label: 'Unowned / untagged', count: eOwn, role: 'medium', onClick: () => go({ tab: 'identities', type: 'Encryption Key' }) },
  ];
  const secretRows: PostureRow[] = [
    { label: 'Unrotated over 90 days', count: secUnrot, role: 'critical', onClick: () => go({ tab: 'identities', type: 'API Key / Secret' }) },
    { label: 'Orphaned / no owner', count: secOrphan, role: 'high', onClick: () => go({ tab: 'identities', type: 'API Key / Secret' }) },
    { label: 'No rotation policy', count: secStale, role: 'medium', onClick: () => go({ tab: 'identities', type: 'API Key / Secret' }) },
  ];
  const protoSlices: DonutSlice[] = [
    { label: 'SSL 3.0', count: ssl, stroke: CORAL, text: 'text-coral', onClick: () => go({ tab: 'crypto-assets', view: 'protocols', q: 'SSL' }) },
    { label: 'TLS 1.0', count: t10, stroke: '#d85a30', text: 'text-coral', onClick: () => go({ tab: 'crypto-assets', view: 'protocols', q: 'TLS 1.0' }) },
    { label: 'TLS 1.1', count: t11, stroke: AMBER, text: 'text-amber', onClick: () => go({ tab: 'crypto-assets', view: 'protocols', q: 'TLS 1.1' }) },
    { label: 'TLS 1.2', count: t12, stroke: MUTED, text: 'text-muted-foreground', onClick: () => go({ tab: 'crypto-assets', view: 'protocols', q: 'TLS 1.2' }) },
    { label: 'TLS 1.3 / SSH', count: t13 + sshP, stroke: TEAL, text: 'text-teal', onClick: () => go({ tab: 'crypto-assets', view: 'protocols', q: 'TLS 1.3' }) },
  ];
  const libSlices: DonutSlice[] = [
    { label: 'End-of-life', count: lEol, stroke: CORAL, text: 'text-coral', onClick: () => go({ tab: 'crypto-assets', view: 'libraries', eol: 'End-of-Life' }) },
    { label: 'Outdated', count: lOut, stroke: AMBER, text: 'text-amber', onClick: () => go({ tab: 'crypto-assets', view: 'libraries', eol: 'Outdated' }) },
    { label: 'Supported', count: lSup, stroke: TEAL, text: 'text-teal', onClick: () => go({ tab: 'crypto-assets', view: 'libraries' }) },
  ];
    ...
    <div className="bg-card rounded-xl border border-border p-5">
      <div className="flex items-center justify-between mb-3">
        <div>
          ...
          <p className="text-[10px] text-muted-foreground">Every number is a count. Click to drill into inventory with filters pre-applied.</p>
        </div>
        <span className="text-[10px] text-muted-foreground">6 categories</span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        <PostureTile icon={FileBadge} label="Certificates" total={certs.length}
        ...
        <PostureTile icon={FileKey} label="SSH keys" total={ssh.length}
        ...
        <PostureTile icon={Key} label="Encryption keys" total={enc.length}
        ...
        <PostureTile icon={Lock} label="Secrets" total={secrets.length}
        ...
        <PostureTile icon={Network} label="Protocols" total={mockProtocols.length}
        ...
        <PostureTile icon={Package} label="Libraries" total={mockLibraries.length}
