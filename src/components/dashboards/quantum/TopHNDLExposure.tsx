...
export default function TopHNDLExposure() {
  ...
  return (
    <div className="bg-card rounded-xl border border-border p-5 space-y-4">
      ...
      <div className="space-y-2">
        {q.topObjects.map(o => (
          <div key={o.id} className="flex items-center justify-between gap-3 py-2 border-b border-border/40 last:border-0">
            ...
            <span className={`px-2 py-0.5 rounded text-[10px] font-bold tabular-nums ${sevColor(o.qoe)}`}>
              QOE {o.qoe}
            </span>
          </div>
        ))}
        {q.topObjects.length === 0 && (
          <p className="text-[11px] text-muted-foreground py-4 text-center">No quantum-vulnerable objects in inventory.</p>
        )}
      </div>
    </div>
  );
