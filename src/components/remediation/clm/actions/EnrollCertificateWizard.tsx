...
const certificateAuthorities = [
  'Internal Native CA', 'DigiCert', 'DigiCert One', 'Sectigo', 'Microsoft Enterprise', "Let's Encrypt", 'GlobalSign',
            ...
            <Accordion type="multiple" defaultValue={[...sectionIds]} className="space-y-3">
              ...
              <AccordionItem value="ca-details" className="overflow-hidden rounded-lg border border-border bg-background/30">
                ...
                <AccordionContent className="space-y-4 px-4 pt-4">
                  ...
                  <AnimatedBlock show={ca === 'Google CA' || ca === 'Internal Native CA'}><div className="grid gap-4 pt-3 md:grid-cols-2"><SelectField label="Issuer Name" value={issuerName} onChange={setIssuerName} options={['issuer-prod', 'issuer-staging']} /></div></AnimatedBlock>
                  <AnimatedBlock show={ca === 'Internal Native CA'}><div className="grid gap-4 pt-3 md:grid-cols-2"><TextField label="Template Name" value={templateName} onChange={(event) => setTemplateName(event.target.value)} /></div></AnimatedBlock>
                    ...
                    <div className="space-y-4 pt-3">
                      <FieldShell label="CSR Generation" required note="Not applicable for Amazon CA">
                        <RadioGroup value={csrGeneration} onValueChange={(value) => setCsrGeneration(value as (typeof csrGenerationOptions)[number])} className="grid gap-3">
                          {[
                            { value: 'platform', label: 'the platform', description: 'Private key and CSR created in the platform', disabled: false },
              ...
              <AccordionItem value="attributes" className="overflow-hidden rounded-lg border border-border bg-background/30">
                ...
                <AccordionContent className="space-y-4 px-4 pt-4">
                  <div className="rounded-md border border-teal/20 bg-teal/5 px-3 py-2 text-[11px] text-muted-foreground">These values are stored in the platform inventory and can be used for filtering. They are not part of the certificate itself.</div>
                  {attributes.map((row) => (
