...
export interface ITAsset {
  id: string;
  name: string;
  // Layer 1: IT Asset Class (the only top-level types).
  assetClass: "Host" | "Application" | "Database" | "API Gateway" | "Kubernetes Workload";
  // Layer 2: Role (attribute, optional). For Host: Web Server, Mail Server, Bastion, etc.
  role?: string;
  // Layer 3: Technology (attribute, optional). Apache, NGINX, Oracle, Spring Boot, etc.
  technology?: string;
  // Where this asset record came from: network/endpoint Discovery, or the CMDB.
  source: "Discovery" | "CMDB";
  scanned: boolean;
  ...
  application: string;
  // Deprecated governance fields, retained for the ARS/ERS engines only.
  // Not surfaced in the UI: IT assets are carriers, not governed entities.
