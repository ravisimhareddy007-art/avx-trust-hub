            ...
            <Section title="Push Details">
              <FieldShell label="Script Location" required>
                <RadioGroup value={values.scriptLocation} onValueChange={(value) => setField('scriptLocation', value as 'platform' | 'device')} className="flex gap-6">
                  <div className="flex items-center gap-2"><RadioGroupItem value="platform" id="script-platform" /><Label htmlFor="script-platform">In the platform</Label></div>
...
function CertificateDetailsModal({
  ...
  const rows = [
    ['Common name', title], ['Serial number', '7F:46:98:1A:11:09:AE'], ['Valid from (GMT)', '10/10/2022'], ['Valid to (GMT)', '10/05/2042'], ['Issuer', 'Internal CA'], ['Version', '3'], ['Signature alg', 'SHA256withRSA'], ['Sig hash alg', 'SHA256'], ['Public key alg', 'RSA 2048'], ['ECDSA Curve', 'Not applicable'],
    ['Thumbprint algorithm', 'SHA-1'], ['Thumbprint', '2C:96:81:AF:09:44'], ['Authority key id', 'Not applicable'], ['Certificate authority', 'the platform'], ['Subject country', 'US'], ['Subject locality', 'Seattle'], ['Subject organization', 'Acme Inc'], ['Subject org unit', 'Not applicable'], ['Subject state', 'Washington'], ['Issuer country', 'US'],
  ];
...
export default function CertHolisticView() {
  ...
  const chainCards = useMemo(() => {
    ...
    return [
      { label: 'CA', title: certificate?.caIssuer || 'Internal CA', group: 'Certificate-Gateway', validFrom: '10/10/2022', validTo: '10/05/2042', border: 'border-l-teal' },
        ...
        <div className="mt-8 border-t border-dashed border-border pt-4">
          ...
          {showConnectorRow && (
            <div className="flex flex-wrap items-center justify-center gap-4">
              <div className="rounded-lg border border-border bg-background/70 px-4 py-3 text-sm font-semibold text-foreground">the platform</div>
