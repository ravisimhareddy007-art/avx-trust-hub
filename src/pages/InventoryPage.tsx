...
export default function InventoryPage() {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Bulk actions */}
      {selectedRows.size > 0 && (
        <div className="bg-teal/5 border border-teal/20 rounded-lg px-4 py-2 flex items-center gap-3">
          <span className="text-xs font-medium text-teal">{selectedRows.size} selected</span>
          {['Bulk Renew', 'Assign Owner', 'Add Tag', 'Add to Workflow'].map(action => (
            <button key={action} onClick={() => toast.success(`${action} applied to ${selectedRows.size} assets`)} className="text-[10px] px-2 py-1 rounded bg-teal/10 text-teal hover:bg-teal/20">{action}</button>
          ))}
        </div>
      )}
