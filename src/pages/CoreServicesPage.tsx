...
import { Zap, BarChart3, ExternalLink, Plus, Play, Download, FileText } from 'lucide-react';

export default function CoreServicesPage() {
  const [tab, setTab] = useState<'automation' | 'reporting' | 'self-service'>('automation');
        ...
        <div className="space-y-4">
          ...
          <div className="bg-card rounded-lg border border-border p-8 text-center">
            <div className="border-2 border-dashed border-border rounded-lg p-12">
              <p className="text-sm text-muted-foreground">Visual Workflow Canvas</p>
              <p className="text-[10px] text-muted-foreground mt-1">Select a template above or drag nodes to build a workflow</p>
            </div>
          </div>
        </div>
      )}
      ...
      {tab === 'self-service' && (
        <div className="space-y-4">
          <div className="flex justify-end">
            <button onClick={() => toast.info('New portal wizard opened')} className="flex items-center gap-1 px-3 py-2 rounded-lg bg-teal text-primary-foreground text-xs hover:bg-teal-light"><Plus className="w-3 h-3" /> New Portal</button>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
