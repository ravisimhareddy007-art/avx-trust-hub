...
import { RefreshCw, RotateCcw, XCircle, Shield, Search, Download, CheckCircle2 } from 'lucide-react';
...
function getRemediationItems(assets: CryptoAsset[]): RemediationItem[] {
  ...
  assets.forEach(asset => {
    ...
    if (asset.pqcRisk === 'Critical') {
      items.push({
        asset,
        issue: `PQC vulnerable: ${asset.algorithm}`,
        severity: 'Critical',
        recommendedAction: 'Migrate to post-quantum algorithm (ML-DSA)',
        actionType: 'Migrate',
      });
    }
    ...
    if (asset.status === 'Orphaned') {
      items.push({
        ...
        severity: 'High',
        recommendedAction: 'Assign owner or revoke',
