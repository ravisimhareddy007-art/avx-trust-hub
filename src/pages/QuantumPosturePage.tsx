...
//
// A projection over platform outputs. It computes nothing authoritative:
// QES comes from RiskContext; objects come from the inventory; QOE and the
// agility verdict come from the scoring lib. Every list is the real estate,
// ranked and paginated - not a hardcoded slice. Every row drills into
// inventory; every Raise uses the shared ticket flow.
...
export default function QuantumPosturePage() {
  ...
  const goVulnerable = () => { setFilters({ tab: "identities", quantumVulnerable: "true" }); setCurrentPage("inventory"); };
  const goBlocked = () => { setFilters({ tab: "identities", filterId: "agility-blocked" }); setCurrentPage("inventory"); };
  const goObject = (name: string) => { setFilters({ tab: "identities", search: name }); setCurrentPage("inventory"); };
  const goLibraries = () => { setFilters({ tab: "crypto-assets" }); setCurrentPage("inventory"); };
  ...
  return (
    <div className="p-6 space-y-6 max-w-[1400px]">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {vulnerable.length} quantum-vulnerable objects &middot; ranked by exposure &middot; priced against {profile.label}
          </p>
        </div>
        <div className="flex items-end gap-4">
          <div>
            <label className="text-[9px] uppercase tracking-wider text-muted-foreground font-semibold block mb-1">Deadline profile</label>
            <select
              value={profileId}
          ...
          <div>
            <label className="text-[9px] uppercase tracking-wider text-muted-foreground font-semibold block mb-1">Assumed Q-day</label>
            <select
              value={qDay}
              onChange={(e) => setQDay(Number(e.target.value))}
              className="bg-card border border-border rounded-lg px-3 py-1.5 text-xs text-foreground"
            >
              {[2030, 2032, 2035, 2040].map((y) => (<option key={y} value={y}>{y}</option>))}
            </select>
          </div>
        </div>
      </div>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
            <button onClick={() => setSortKey("qoe")} className={sortKey === "qoe" ? "text-teal font-semibold" : ""}>Sort: exposure</button>
            <button onClick={() => setSortKey("name")} className={sortKey === "name" ? "text-teal font-semibold" : ""}>Sort: name</button>
          </div>
        </div>
        <div className="bg-card rounded-xl border border-border overflow-hidden">
              ...
              {pageRows.map(({ asset, qoe, agility }) => (
                <tr key={asset.id} className="border-b border-border/50 last:border-0 hover:bg-secondary/30">
                  ...
                  <td className="py-2 px-3 text-right">
                    <span className={\`text-[10px] font-bold tabular-nums px-2 py-0.5 rounded \${sevChip(qoe)}\`}>{qoe}</span>
                  </td>
                  <td className="py-2 px-3">
                    {agility.agile ? (
                  ...
                  <td className="py-2 px-3 text-right">
                    <button
                      ...
                      title={agility.hardBlocked ? "Upgrade the blocking library first - a cert change cannot fix this object." : "Raise a change request"}
          ...
          <div className="flex items-center justify-between px-3 py-2 border-t border-border text-[10px] text-muted-foreground">
            <span>
              {vulnerable.length === 0 ? "No quantum-vulnerable objects" :
                \`\${page * PAGE + 1}-\${Math.min((page + 1) * PAGE, vulnerable.length)} of \${vulnerable.length}\`}
            </span>
            <div className="flex items-center gap-2">
              <button disabled={page === 0} onClick={() => setPage((p) => p - 1)} className="disabled:opacity-40"><ChevronLeft className="w-4 h-4" /></button>
              <span>{page + 1} / {pageCount}</span>
              <button disabled={page >= pageCount - 1} onClick={() => setPage((p) => p + 1)} className="disabled:opacity-40"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-[11px] uppercase tracking-wider text-muted-foreground font-semibold">Unblock before you migrate</h2>
            ...
            <tbody>
              {blockers.map((b) => (
                <tr key={b.library} className="border-b border-border/50 last:border-0 hover:bg-secondary/30">
                  <td className="py-2 px-3">
                    <button onClick={goLibraries} className="text-left hover:text-teal font-medium text-foreground">{b.library}</button>
                  </td>
      ...
      <section className="flex items-center justify-between bg-card rounded-xl border border-border p-4">
        <div>
          ...
          <p className="text-[11px] text-muted-foreground mt-0.5">CycloneDX CBOM: every object, its QOE, posture, deadline, blocker and change-request ID.</p>
        </div>
        <div className="flex items-center gap-2">
          <button onClick={() => setCurrentPage("tickets")} className="inline-flex items-center gap-1 text-[11px] text-teal hover:text-teal/80 font-medium">
            Tickets <ChevronRight className="w-3.5 h-3.5" />
          </button>
          <button
            onClick={() => downloadEvidencePack(buildEvidencePack(mockAssets, mockProtocols, mockLibraries, profileId, qDay))}
